...
Running with `bound: false` (native-only mode): there's no bound key to verify a per-request polyfill proof against, but `requireProof()` does **not** simply trust the cookie. As of v2.14 (every adapter) it switches to the **native** per-request proof — the `freshProof` 403-challenge handshake. A proofless request on a `dbsc` session gets `403` + `Secure-Session-Challenge`; Chrome re-signs with the TPM/Secure Enclave key and retries with `Secure-Session-Response`, verified against the stored native key. A stolen cookie has no hardware key, so it's rejected per request — the refresh-cycle replay window is closed without the polyfill. This verifies key *possession*, not body integrity (for body-hash binding you still want the polyfill `bound` tier). Set `requireProof({ freshProof: false })` to restore the old relax-to-cookie behavior; an explicit `allowDbscWithoutProof: true` still takes precedence. See [polyfill.md](./polyfill.md#disabling-the-polyfill-bound-false).
